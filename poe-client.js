  ...
  function g(e, t) {
    return (function (e, t) {
      ...
      for (
        ...
        (a[r] = 909522486 ^ i[r]), (s[r] = 1549556828 ^ i[r]);
      return (
        (n = d(a.concat(h(t)), 512 + 8 * t.length)), f(d(s.concat(n), 640))
      );
    })(_(e), _(t));
  }
  function m(e, t, r) {
...
async function request_with_retries(method, attempts = 10) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      logger.warn(
        `Server returned a status code of ${
          response.status
        } while downloading ${url}. Retrying (${i + 1}/${attempts})...`
      );
    } catch (err) {
      console.log(err);
    }
...
class Client {
  ...
  async init(token, proxy = null) {
    ...
    [this.next_data, this.channel] = await Promise.all([
      this.get_next_data(),
  ...
  async get_next_data() {
    ...
    const r = await request_with_retries(() =>
      this.session.post(this.home_url)
    );
    const jsonRegex =
    ...
    if (!viewerPath || !botNamePath || !defaultBotPath) {
      ...
      console.log(
        "SEARCH THIS LIST FOR 'chatOfBotDisplayName', 'viewer', AND 'defaultBotNickname'..."
      );
      console.log("-----------------");
    ...
    for (const bot of botList.filter((x) => x.deletionState == "not_deleted")) {
      const promise = new Promise(async (resolve, reject) => {
        try {
          ...
          const chatData =
            r.data.pageProps.payload?.chatOfBotDisplayName ||
            r.data.pageProps.data?.chatOfBotHandle;
  ...
  async get_channel_data(channel = null) {
    ...
    const r = await request_with_retries(() =>
      this.session.get(this.settings_url)
    );
    const data = r.data;
  ...
  async send_query(queryName, variables, queryDisplayName) {
    for (let i = 0; i < 20; i++) {
      ...
      _headers["poe-tag-id"] = md5()(
        scramblePayload + this.formkey + "WpuLMiXEKKE98j56k"
      );
      _headers["poe-formkey"] = this.formkey;
      const r = await request_with_retries(() =>
        this.session.post(this.gql_url, payload, { headers: this.gql_headers })
      );
      if (!r?.data?.data) {
  ...
  async *send_message(
    ...
    try {
      ...
      throw new Error(
        `An unknown error occured. Raw response data: ${messageData}`
      );
    }
